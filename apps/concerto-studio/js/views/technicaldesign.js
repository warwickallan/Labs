/* technicaldesign.js — TECHNICAL DESIGN: the consultant's exhaustive
 * generated report, in the ORIGINAL Solution Design format (soldesign.js).
 * It may legitimately contain known faults, gaps, register codes and
 * evidence grading — that is its job. The customer-facing Solution Design
 * is the separate view next to this one.
 *
 ...
 * the document describes THAT engagement. The generic Vanilla edition
 * belongs to Settings → Vanilla.
 */
(function () {
  'use strict';
  ...
  function render(container, base, opts) {
    ...
    page.appendChild(el('div', { class: 'toolstrip' }, [
      ...
      project ? el('span', { class: 'src-chip', text: 'Internal — may contain known faults, gaps and register codes. Vanilla edition: Settings → Vanilla' }) : null,
      ...
      el('button', {
        ...
        onclick: function () {
          ...
          a.download = ((project ? project.key.toUpperCase() + '-' : '') + String(state.edition || 'vanilla').toUpperCase()) + '-TECHNICAL-DESIGN.html';
          a.click();
    ...
    var frame = el('iframe', {
      ...
      title: 'Technical Design preview'
    });
    frame.setAttribute('srcdoc', buildDocument(base, opts));
    page.appendChild(frame);
  }

  window.StudioTechnicalDesignView = { render: render, _state: state };
})();
